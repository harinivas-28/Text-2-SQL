...
def get_prompt(tables, question):
    prompt = f"""Generate a MySQL query for the given question using the table 'table_name' and the provided columns. The query should be one of the following types: simple MySQL query, aggregated MySQL query, subquery and query must not contain join functions and SELECT * if ask for 'all'. Table Columns: {tables}. Question: {question}"""
    return prompt
...
def plot_result(result: pd.DataFrame, output_dir: str):
    ...
    if num_columns == 1:
        ...
        if pd.api.types.is_numeric_dtype(result[column]):
            ...
            if result[column].dtype == 'object':
                ...
                plt.title(f'Distribution of {column}')
                plot_path = os.path.join(output_dir, f'{column}_piechart.png')
                plt.savefig(plot_path)
                plot_paths.append(plot_path)
                plt.close()
            else:
    ...
    # Two Columns: Bar Plot, Scatter Plot, or Count Plot
        ...
        if pd.api.types.is_numeric_dtype(result[x_col]) and pd.api.types.is_numeric_dtype(result[y_col]):
            ...
            plot_path = os.path.join(output_dir, f'{x_col}_vs_{y_col}_scatter.png')
            plt.savefig(plot_path)
            plot_paths.append(plot_path)
            plt.close()
            ...
            plot_path = os.path.join(output_dir, f'{y_col}_by_{x_col}_bar.png')
            plt.savefig(plot_path)
            plot_paths.append(plot_path)
            plt.close()
            ...
            plot_path = os.path.join(output_dir, f'{y_col}_count_by_{x_col}_count.png')
            plt.savefig(plot_path)
            plot_paths.append(plot_path)
            plt.close()
